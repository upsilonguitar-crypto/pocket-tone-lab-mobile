...
function selectPreset(id,opts={scroll:true}){const p=state.presets.find(x=>x.id===id);if(!p)return;state.current=structuredClone(p);renderPresets();renderDetail(state.current,$('#detailPanel'));updateHardwareStrip(state.current);if(opts.scroll&&innerWidth<900)$('#detailPanel').scrollIntoView({behavior:'smooth',block:'start'})}
...
function importPrstToStudio(){
  ...
  state.current=structuredClone(p);renderDetail(state.current,$('#detailPanel'));updateHardwareStrip(state.current);activateTab('library');toast('Preset natif chargé dans Tone Studio');
}
